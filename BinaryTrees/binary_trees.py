...
class SinglyLinkedList:
    ...
    def recursive_find(self, data):
        ...
        Returns:
            (SinglyLinkedListNode): the node containing the data.
        """
        # Define a function that finds the node corresponding to the given data
        def check_data(current):
            if current.value == None:
                raise ValueError(str(data) + " is not in the list") # Raise error if data is not in list
            if current.value == data:
                return current          # Return the appropriate node when found
            else:
                return check_data(current.next)  # Check the next node if current node is not the right one
        # Start the recursion with the head
...
class BST:
    ...
    def insert(self, data):
        ...
        Example:
            ...
            [8]                                 |
        """
        # Define a function to find the parent node of the data and insert the new data below it
        def find_parent(new, current):
        # Raise error if 'new' node is already in the tree
            if new.value == current.value:
                raise ValueError("Value is repeated")
        # If 'new' is less than current node and the current node has no left leaf insert 'new',
        # if it does have a left leaf repeat the function with that node as the parent
            if new.value < current.value:
                if current.left == None:
                    ...
                    find_parent(new, current.left)
        # If 'new' is greater than current node and the current node has no right leaf insert 'new',
        # if it does have a right leaf repeat the function with that node as the parent
            if new.value > current.value:
                if current.right == None:
                    ...
                    find_parent(new, current.right)
        # If the tree is empty, make the new data the root node
        if self.root == None:
            new = BSTNode(data)
            self.root = new
        # Make data a node, and use find_parent() starting at the root to insert it
        else:
            new = BSTNode(data)
            find_parent(new, self.root)
...
if __name__ == "__main__":
    ...
    for i in [4, 3, 6, 9, 7, 8, 1]:
        list.append(i)
    print(SinglyLinkedList.recursive_find(list,6))