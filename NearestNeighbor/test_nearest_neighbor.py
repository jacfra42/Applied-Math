...
def test_KDTNode():
    ...
    """
    pytest.raises(TypeError, nearest_neighbor.KDTNode(4)), "x is not of type np.ndarray"
    node = nearest_neighbor.KDTNode(np.array[1,2,3,4])
    assert (node.value == np.array[1,2,3,4]), "Incorrect value returned"
